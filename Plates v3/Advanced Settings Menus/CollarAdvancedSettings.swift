...
class CollarAdvancedSettings: UITableViewController, UITextFieldDelegate {
    ...
    var currentcollarIndex = Int()

    @IBAction func addButtonAction(_ sender: AnyObject) {
        if app.profile.collarCollection.list.count <= 8 {
            let newcollar = AppData.Collar(name: findNextName(), unitType:
                PublicClasses.currentLocaleUnit.unit, weight: 0)
            app.profile.collarCollection.list.insert(newcollar, at: 0)
    ...
    @objc func segmentValueChanged(_ sender: UISegmentedControl) {
        ...
        if sender.selectedSegmentIndex == 0 {
            app.profile.collarCollection.list[sender.tag].unitType = UnitOfWeight.unitType.kg
        }
        else if sender.selectedSegmentIndex == 1 {
            app.profile.collarCollection.list[sender.tag].unitType = UnitOfWeight.unitType.lb
        }
        updateGlobals()
        self.tableView.reloadData()
    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        currentcollarIndex = app.profile.collarCollection.list.index(where: {$0.equals(compareTo: app.profile.currentCollar)})!
        print("Current Collar Index: \(currentcollarIndex)")
    ...
    func findNextName() -> String {
        ...
        if app.profile.collarCollection.list.count > 0 {
            for i in 0...app.profile.collarCollection.list.count-1 {
                let collarNameToSplit: String = app.profile.collarCollection.list[i].name
                ...
                if collarNameToSplit.count >= collarSubstring.count {
                    ...
                    let index = app.profile.collarCollection.list[i].name.replacingOccurrences(of: collarSubstring, with: "", options: NSString.CompareOptions.literal, range: nil).trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
    ...
    func updateGlobals() {
        if (app.profile.collarCollection.list.index(where: {$0.equals(compareTo: app.profile.currentCollar)})) == nil {
            if !(app.profile.collarCollection.list[currentcollarIndex].name == app.profile.currentCollar.name ||
                app.profile.collarCollection.list[currentcollarIndex].weight == app.profile.currentCollar.weight) {
                self.currentcollarIndex = app.profile.collarCollection.list.endIndex-1
            }
            print(self.currentcollarIndex)
            app.profile.currentCollar = app.profile.collarCollection.list[self.currentcollarIndex]
            app.profile.chosenUnit.unit = app.profile.collarCollection.list[self.currentcollarIndex].unitType
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if (textField.tag == 2) && ((textField.text?.count)! > 0) {
            textField.text = app.profile.collarCollection.list[(textField.superview?.superview?.tag)!].weight.clean
        }
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        if textField.tag == 1 { //Name textfield
            ...
            if textField.text!.count == 0 || textField.text == NSLocalizedString("No collar", comment: "") {
                textField.text! = findNextName()
            }
            app.profile.collarCollection.list[row].name = textField.text!
            textField.text = app.profile.collarCollection.list[row].name
        }
            ...
            textField.text = PublicClasses.massFormatter.string(fromValue: newWeight, unit: UnitOfWeight(unit:  app.profile.collarCollection.list[row].unitType).formatter)
            app.profile.collarCollection.list[row].weight = newWeight
        }
        self.updateGlobals()
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if app.profile.collarCollection.list[row].name != NSLocalizedString("No collar", comment: "") {
            cell.tag = row
            cell.nameTextField.text = NSLocalizedString(app.profile.collarCollection.list[row].name, comment: "")
            ...
            if app.profile.collarCollection.list[row].unitType == UnitOfWeight.unitType.kg {
            ...
            cell.valueTextField.text = PublicClasses.massFormatter.string(fromValue: app.profile.collarCollection.list[row].weight, unit: unitToPass)
    ...
    override func tableView(_ tableView: UITableView, editingStyleForRowAt indexPath: IndexPath) -> UITableViewCellEditingStyle {
        if app.profile.currentCollar.name != NSLocalizedString("No collar", comment: "") {
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let delete = UITableViewRowAction(style: .destructive, title: "delete") { (action, indexPath) in
            app.profile.collarCollection.list.remove(at: (indexPath as NSIndexPath).row)
            self.updateGlobals()
